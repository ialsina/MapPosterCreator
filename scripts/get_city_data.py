...
def fetch_data():
    paths.cities_gh_datasets.parent.mkdir(parents=True, exist_ok=True)
    wget.download(DATA_URL, out=str(paths.cities_gh_datasets))
    with open(paths.cities_gh_datasets_hash, "w", encoding="utf-8") as hf:
        hf.write(get_github_commit_hash())

def read_cities_df():
    return read_csv(paths.cities_gh_datasets)
